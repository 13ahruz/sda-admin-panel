...
class Project(TimestampMixin):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.cover_photo:
            self.cover_photo_url = f"http://localhost:8000/uploads/{self.cover_photo.name}"
...
class ProjectPhoto(TimestampMixin):
    ...
    def save(self, *args, **kwargs):
        if self.image:
            self.image_url = f"http://localhost:8000/uploads/{self.image.name}"
        super().save(*args, **kwargs)
...
class Service(TimestampMixin):
    ...
    def save(self, *args, **kwargs):
        if self.icon:
            self.icon_url = f"http://localhost:8000/uploads/{self.icon.name}"
        super().save(*args, **kwargs)
...
class TeamMember(TimestampMixin):
    ...
    def save(self, *args, **kwargs):
        if self.photo:
            self.photo_url = f"http://localhost:8000/uploads/{self.photo.name}"
        super().save(*args, **kwargs)
...
class WorkProcess(TimestampMixin):
    ...
    def save(self, *args, **kwargs):
        if self.image:
            self.photo_url = f"http://localhost:8000/uploads/{self.image.name}"
        super().save(*args, **kwargs)
